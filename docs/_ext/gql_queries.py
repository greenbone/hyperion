...
class LoadModule(Container):
    ...
    def run(self):
        ...
        queries = schema_module.schema.get_query_type()
        node = build_queries(queries, self.state)
        return node


def build_queries(queries, state):
    """Iteration over the different module fields and create
    a table for each class"""
    tables = []
    for item in queries.fields.items():
...
def setup(app):
    app.add_directive('gql-queries', LoadModule)
