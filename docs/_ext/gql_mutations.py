...
class LoadModule(Container):
    ...
    def run(self):
        ...
        queries = schema_module.schema.get_mutation_type()
        node = build_mutations(queries, self.state)
        return node


def build_mutations(mutations, state):
    """Iteration over the different module fields and create
    a table for each class"""
    tables = []
    for item in mutations.fields.items():
...
def table_container(item, state):
    ...
    if hasattr(item[1], 'args'):
        if len(item[1].args.items()) > 0:
...
def setup(app):
    app.add_directive('gql-mutations', LoadModule)
