...
class ExportTasksByFilterTestCase(SeleneTestCase):
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportTasksByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_tasks_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        xml = json['data']['exportTasksByIds']['exportedEntities']
