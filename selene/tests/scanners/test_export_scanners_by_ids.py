...
class ExportScannersByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportScannersByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_scanners_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_scanners_response status="200" status_text="OK">'
            f'<scanner id="{self.id1}">'
            '<name>some_name1</name>'
            '</scanner>'
            f'<scanner id="{self.id2}">'
            '<name>some_name2</name>'
            '</scanner>'
            '</get_scanners_response>'
        )

        mock_gmp.mock_response('get_scanners', mock_xml)
        ...
        scanners_xml = json['data']['exportScannersByIds']['exportedEntities']

        self.assertEqual(mock_xml, scanners_xml)
