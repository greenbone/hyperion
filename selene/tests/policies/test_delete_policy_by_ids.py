...
class DeletePoliciesByIdsTestCase(SeleneTestCase):
    ...
    def test_delete_policies_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        mock_gmp.gmp_protocol.delete_policy.assert_any_call(
            policy_id=str(id1), ultimate=True
        )

        mock_gmp.gmp_protocol.delete_policy.assert_any_call(
            policy_id=str(id2), ultimate=True
        )
