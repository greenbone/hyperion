...
class ExportPoliciesByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportPoliciesByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_policies_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        mock_gmp.mock_response('get_policies', mock_xml)
        ...
        policies_xml = json['data']['exportPoliciesByIds']['exportedEntities']

        self.assertEqual(mock_xml, policies_xml)
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_gmp.mock_response('get_policies', bytes(mock_xml, 'utf-8'))

        response = self.query(
            '''
            mutation {
                exportPoliciesByIds(ids: []) {
                   exportedEntities
                }
            }
            '''
