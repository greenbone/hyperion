...
class ExportSchedulesByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportSchedulesByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_schedules_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_schedules_response status="200" status_text="OK">'
            f'<schedule id="{self.id1}">'
            '<name>some_name1</name>'
            '</schedule>'
            f'<schedule id="{self.id2}">'
            '<name>some_name2</name>'
            '</schedule>'
            '</get_schedules_response>'
        )

        mock_gmp.mock_response('get_schedules', mock_xml)
        ...
        schedules_xml = json['data']['exportSchedulesByIds']['exportedEntities']

        self.assertEqual(mock_xml, schedules_xml)
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_xml = (
            ...
            'ascending</order></field></sort><schedules max=\"10\" '
            'start=\"1\"/><schedule_count>16<filtered>0</filtered>'
            '<page>0</page></schedule_count></get_schedules_response>'
        )
        ...
        mock_gmp.gmp_protocol.get_schedules.assert_called_with(filter_string='')
