...
class DeleteAlertsByIdsTestCase(SeleneTestCase):
    ...
    def test_delete_alerts(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        ok = json['data']['deleteAlertsByIds']['ok']
    ...
    def test_delete_alerts_invalid(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        id1 = uuid4()
        id2 = uuid4()

        # Only one ID of the requested Ids is found.
        mock_gmp.mock_response(
            'get_alerts',
            f'''
            <get_alerts_response status="200" status_text="OK">
