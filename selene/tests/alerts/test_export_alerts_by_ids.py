...
class ExportAlertsByFilterTestCase(SeleneTestCase):
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportAlertsByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_alerts_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_alerts_response status="200" status_text="OK">'
            f'<apply_overrides>0</apply_overrides><alert id="{self.id1}">'
            f'<name>Foo Clone 1</name></alert><alert id="{self.id2}">'
        ...
        xml = json['data']['exportAlertsByIds']['exportedEntities']
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_xml = (
            ...
            "<value/></keyword></keywords></filters></get_alerts_response>"
        )
        ...
        mock_gmp.gmp_protocol.get_alerts.assert_called_with(filter_string='')
