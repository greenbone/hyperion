...
class ExportCredentialsByFilterTestCase(SeleneTestCase):
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            '''
            mutation {
                exportCredentialsByFilter(filterString: "lorem=ipsum") {
                   exportedEntities
                }
            }
            '''
    ...
    def test_export_credentials_by_filter(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        xml = json['data']['exportCredentialsByFilter']['exportedEntities']
