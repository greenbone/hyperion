...
class DeleteCredentialsByIdsTestCase(SeleneTestCase):
    ...
    def test_delete_credentials_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        response = self.query(
            f'''
            mutation {{
                deleteCredentialsByIds(ids: ["{id1}", "{id2}"],
                    ultimate:true)
                {{
                    ok
                }}
        ...
        ok = json['data']['deleteCredentialsByIds']['ok']
        self.assertTrue(ok)
        ...
        mock_gmp.gmp_protocol.delete_credential.assert_any_call(
            credential_id=str(id1), ultimate=True
        )

        mock_gmp.gmp_protocol.delete_credential.assert_any_call(
            credential_id=str(id2), ultimate=True
        )
    ...
    def test_delete_by_ids_invalid(self, mock_gmp: GmpMockFactory):
        ...
        # Return only one credential instead of the queried two.
        mock_gmp.mock_response(
            'get_credentials',
            f'''
            <get_credential_response status="200" status_text="OK">
