...
class ExportNvtsByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportNvtsByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_nvts_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        oval_definitions_xml = json['data']['exportNvtsByIds'][
            'exportedEntities'
        ]
    ...
    def test_export_nvt_empty_ids_array(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        mock_gmp.gmp_protocol.get_info_list.assert_called_with(
            filter_string='', details=True, info_type=GvmInfoType.NVT
        )
