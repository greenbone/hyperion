...
class DeleteReportFormatsByIdsTestCase(SeleneTestCase):
    ...
    def test_delete_report_formats(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        response = self.query(
            f'''
            mutation {{
                deleteReportFormatsByIds(ids: ["{id1}", "{id2}"]) {{
                    ok
                }}
            }}
            '''
        ...
        ok = json['data']['deleteReportFormatsByIds']['ok']
    ...
    def test_delete_report_formats_invalid(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        id1 = uuid4()
        id2 = uuid4()

        # Only one of the requested report_formats is found.
        mock_gmp.mock_response(
            'get_report_formats',
            f'''
            <get_report_formats_response status="200" status_text="OK">
