...
class ExportTLSCertificatesByFilterTestCase(SeleneTestCase):
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            '''
            mutation {
                exportHostsByFilter(filterString: "lorem=ipsum") {
                   exportedEntities
                }
            }
            '''
    ...
    def test_export_tls_certificates_by_filter(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        mock_xml = (
            ...
            f'<tls_certificate id="{id1}">'
            '<name>some_name1</name>'
            '</tls_certificate>'
            f'<tls_certificate id="{id2}">'
        ...
        xml = json['data']['exportTlsCertificatesByFilter']['exportedEntities']
