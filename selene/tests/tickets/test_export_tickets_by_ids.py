...
class ExportTicketsByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportTicketsByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_tickets_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_tickets_response status="200" status_text="OK">'
            f'<ticket id="{self.id1}">'
            '<name>some_name1</name>'
            '</ticket>'
            f'<ticket id="{self.id2}">'
            '<name>some_name2</name>'
            '</ticket>'
            '</get_tickets_response>'
        )

        mock_gmp.mock_response('get_tickets', mock_xml)
        ...
        tickets_xml = json['data']['exportTicketsByIds']['exportedEntities']

        self.assertEqual(mock_xml, tickets_xml)
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_gmp.gmp_protocol.get_tickets.assert_called_with(filter_string='')
