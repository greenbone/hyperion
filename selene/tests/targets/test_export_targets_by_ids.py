...
class ExportTargetsByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportTargetsByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_targets_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_targets_response status="200" status_text="OK">'
            f'<target id="{self.id1}">'
            '<name>some_name1</name>'
            '</target>'
            f'<target id="{self.id2}">'
            '<name>some_name2</name>'
            '</target>'
            '</get_targets_response>'
        )

        mock_gmp.mock_response('get_targets', mock_xml)
        ...
        targets_xml = json['data']['exportTargetsByIds']['exportedEntities']

        self.assertEqual(mock_xml, targets_xml)
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_xml = (
            ...
            '<targets max=\"10\" start=\"1\"/><target_count>16<filtered>0'
            '</filtered><page>0</page></target_count></get_targets_response>'
        )
        ...
        mock_gmp.gmp_protocol.get_targets.assert_called_with(filter_string='')
