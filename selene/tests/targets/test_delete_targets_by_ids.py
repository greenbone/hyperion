...
class DeleteTargetByIdsTestCase(SeleneTestCase):
    ...
    def test_delete_targets_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        ok = json['data']['deleteTargetsByIds']['ok']
        self.assertTrue(ok)
    ...
    def test_delete_target_by_ids_invalid(self, mock_gmp: GmpMockFactory):
        ...
        # Return only one target instead of the queried two.
        mock_gmp.mock_response(
            'get_targets',
            f'''
            <get_target_response status="200" status_text="OK">
