...
class ExportFiltersByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportFiltersByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_filters_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_filters_response status="200" status_text="OK">'
            f'<filter id="{self.id1}">'
            '<name>some_text1</name>'
            '</filter>'
            f'<filter id="{self.id2}">'
            '<name>some_text2</name>'
            '</filter>'
            '</get_filters_response>'
        )

        mock_gmp.mock_response('get_filters', mock_xml)
        ...
        filters_xml = json['data']['exportFiltersByIds']['exportedEntities']

        self.assertEqual(mock_xml, filters_xml)
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_xml = (
            ...
            'ascending</order></field></sort><filters max=\"10\" '
            'start=\"1\"/><filter_count>16<filtered>0</filtered>'
            '<page>0</page></filter_count></get_filters_response>'
        )
        ...
        mock_gmp.gmp_protocol.get_filters.assert_called_with(filter_string='')
