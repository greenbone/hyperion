...
class DeleteFiltersByFilterTestCase(SeleneTestCase):
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            '''
            mutation {
                deleteFiltersByFilter(filterString: "name~Clone") {
                   ok
                }
            }
            '''
    ...
    def test_delete_filtered_filters(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        ok = json['data']['deleteFiltersByFilter']['ok']
