...
class ImportScanConfigTestCase(SeleneTestCase):
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            '''
            mutation {
                importScanConfig(config: "xml_of_config") {
                   id
                }
            }
            '''
    ...
    def test_import_scan_config(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        response = self.query(
            f'''
            mutation {{
                importScanConfig(config: "{config_xml}")
                {{
                   id
                }}
            }}
        ...
        id_new_config = json['data']['importScanConfig']['id']
