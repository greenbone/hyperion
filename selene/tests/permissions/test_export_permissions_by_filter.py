...
class ExportPermissionsByFilterTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            '''
            mutation {
                exportPermissionsByFilter(filterString: "lorem=ipsum") {
                   exportedEntities
                }
            }
            '''
    ...
    def test_export_permissions_by_filter(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        response = self.query(
            ...
            mutation {{
                exportPermissionsByFilter
                    (filterString: "uuid={self.id1} uuid={self.id2}") {{
                   exportedEntities
                }}
            }}
            '''
        ...
        permissions_xml = json['data']['exportPermissionsByFilter'][
            'exportedEntities'
        ]
