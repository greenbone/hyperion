...
class DeletePermissionsByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_delete_permissions_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        response = self.query(
            f'''
            mutation {{
                deletePermissionsByIds(ids: ["{self.id1}", "{self.id2}"],
                    ultimate:true)
                {{
                    ok
                }}
        ...
        ok = json['data']['deletePermissionsByIds']['ok']
        self.assertTrue(ok)
