...
class ExportOvalDefinitionsByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportOvalDefinitionsByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_oval_definitions_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        oval_definitions_xml = json['data']['exportOvalDefinitionsByIds'][
            'exportedEntities'
        ]
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        response = self.query(
            '''
            mutation {
                exportOvalDefinitionsByIds(ids: []) {
                   exportedEntities
                }
            }
            '''
        ...
        mock_gmp.gmp_protocol.get_info_list.assert_called_with(
            filter='uuid= ', details=True, info_type=GvmInfoType.OVALDEF
        )
