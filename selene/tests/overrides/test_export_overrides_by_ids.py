...
class ExportOverridesByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportOverridesByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_overrides_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')

        mock_xml = (
            '<get_overrides_response status="200" status_text="OK">'
            f'<override id="{self.id1}">'
            '<name>some_text1</name>'
            '</override>'
            f'<override id="{self.id2}">'
            '<name>some_text2</name>'
            '</override>'
            '</get_overrides_response>'
        )

        mock_gmp.mock_response('get_overrides', mock_xml)
        ...
        overrides_xml = json['data']['exportOverridesByIds']['exportedEntities']
    ...
    def test_export_empty_ids_array(self, mock_gmp: GmpMockFactory):
        ...
        mock_xml = (
            ...
            'ascending</order></field></sort><overrides max=\"10\" '
            'start=\"1\" /><override_count>16<filtered>0</filtered>'
            '<page>0</page></override_count></get_overrides_response>'
        )
