...
class ExportCpesByIdsTestCase(SeleneTestCase):
    def setUp(self):
        self.id1 = uuid4()
        self.id2 = uuid4()
    ...
    def test_require_authentication(self, _mock_gmp: GmpMockFactory):
        response = self.query(
            f'''
            mutation {{
                exportCpesByIds(ids: ["{self.id1}", "{self.id2}"]) {{
                   exportedEntities
                }}
            }}
            '''
    ...
    def test_export_cpes_by_ids(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        oval_definitions_xml = json['data']['exportCpesByIds'][
            'exportedEntities'
        ]
    ...
    def test_export_cpe_empty_ids_array(self, mock_gmp: GmpMockFactory):
        self.login('foo', 'bar')
        ...
        mock_gmp.gmp_protocol.get_info_list.assert_called_with(
            filter='uuid= ', details=True, info_type=GvmInfoType.CPE
        )
